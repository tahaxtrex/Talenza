...
import { useN8nPipeline, useN8nScenario, useN8nScorer } from '@/lib/useN8n';
import type { ScenarioQuestion } from '@/lib/n8nService';

const ease = [0.16, 1, 0.3, 1] as const;

type Step = 'candidates' | 'scenario' | 'qa' | 'scoring';

export default function ExternalPipeline() {
  const [step, setStep] = useState<Step>('candidates');
  const [candidates, setCandidates] = useState<CandidateProfile[]>(() => loadPipelineCandidates('external'));
  const [selectedExistingIds, setSelectedExistingIds] = useState<Set<string>>(() => new Set(candidates.map(c => c.id)));
  ...
  const toggleExisting = (candidate: any) => {
    ...
    if (newSet.has(candidate.id)) {
      ...
      savePipelineCandidates('external', updated);
    } else {
      newSet.add(candidate.id);
      const profile: CandidateProfile = {
        id: candidate.id, type: 'external', name: candidate.name, role: candidate.role,
        headline_strength: candidate.headline_strength, headline_risk: candidate.headline_risk,
  ...
  const selectAll = () => {
    ...
    const profiles: CandidateProfile[] = existingCandidates.map(c => ({
      id: c.id, type: 'external' as const, name: c.name, role: c.role,
      headline_strength: c.headline_strength, headline_risk: c.headline_risk,
  ...
  const processCandidate = async () => {
    ...
    try {
      ...
      const { profile } = await n8nIngest(
        'external',
        cvText,
        undefined, // no HR opinion for external
        undefined,
  ...
  const parseScenario = async () => {
    ...
    try {
      const result = await n8nAnalyze('external', scenarioText);
  ...
  const submitQAAnswer = async () => {
    ...
    if (nextIdx < n8nQuestions.length) {
      ...
      // All answered — send to QA webhook
      setScenarioParsing(true);
      setCurrentQA(null);
      try {
  ...
  const runScoring = async () => {
    ...
    setScoring(true);
    try {
      const { report } = await n8nScore('external', candidates, scenario);
      ...
      if (report.mock) {
        ...
        const mapped: ScoringResult[] = rankedList.map((c: any, i: number) => ({
          ...
          rank: c.rank || i + 1,
          breakdown: { scenario_fit: 0, experience_match: 0, leadership_fit: 0, availability: 0, risk_factor: 0 },
          dimensions: [],
      ...
      <div className="max-w-[900px] mx-auto px-6 py-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease }}>
          ...
          <p style={{ fontFamily: 'var(--font-body)' }} className="text-[15px] text-[hsl(var(--color-text-secondary))] mt-2">
            Upload external candidate CVs, define the scenario, and score them.
          </p>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
              ...
              {existingCandidates.length > 0 && (
                <div className="bg-[hsl(var(--color-surface))] border border-[hsl(var(--color-border))] rounded-2xl p-6 mb-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 style={{ fontFamily: 'var(--font-display)' }} className="text-[22px] text-[hsl(var(--color-text-primary))]">Select Existing Candidates</h2>
                    <button onClick={selectAll} className="text-[13px] font-medium text-[hsl(var(--color-accent))] hover:underline" style={{ fontFamily: 'var(--font-body)' }}>
                      ...
                      return (
                        ...
                        >
                          <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${
                            isSelected ? 'bg-[hsl(var(--color-accent))] border-[hsl(var(--color-accent))]' : 'border-[hsl(var(--color-border-strong))]'
                          }`}>
                          ...
                          <span className="text-[11px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-md bg-[hsl(var(--color-accent-light))] text-[hsl(var(--color-accent))]" style={{ fontFamily: 'var(--font-body)' }}>External</span>
                        </motion.button>
                      );
                    })}
                  </div>
              ...
              {!showAddForm ? (
                ...
                <div className="bg-[hsl(var(--color-surface))] border border-[hsl(var(--color-border))] rounded-2xl p-6 space-y-5">
                  <div className="flex items-center justify-between">
                    <h2 style={{ fontFamily: 'var(--font-display)' }} className="text-[22px] text-[hsl(var(--color-text-primary))]">Add External Candidate</h2>
                    <button onClick={() => setShowAddForm(false)} className="text-[hsl(var(--color-text-tertiary))] hover:text-[hsl(var(--color-text-primary))]"><X size={18} /></button>
                  </div>

                  <div
                    className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 ${
                      dragOver ? 'border-[hsl(var(--color-accent))] bg-[hsl(var(--color-accent))]/5' : cvFile ? 'border-[hsl(var(--color-success))] bg-[hsl(var(--color-success-light))]' : 'border-[hsl(var(--color-border-strong))] hover:border-[hsl(var(--color-accent))]/50'
                    }`}
                  ...
                  >
                    {cvFile ? (
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <button onClick={e => { e.stopPropagation(); setCvFile(null); }} className="ml-2"><X size={16} className="text-[hsl(var(--color-text-tertiary))] hover:text-[hsl(var(--color-danger))]" /></button>
                      </div>
                    ) : (
                      <>
                        ...
                        <p style={{ fontFamily: 'var(--font-body)' }} className="text-[12px] text-[hsl(var(--color-text-tertiary))] mt-1">Optional — fill in details manually</p>
                      </>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      onClick={() => {
                        ...
                        savePipelineCandidates('external', []);
                      }}
            ...
            <motion.div key="scenario" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.35, ease }}>
              <div className="bg-[hsl(var(--color-surface))] border border-[hsl(var(--color-border))] rounded-2xl p-6">
                ...
                <p style={{ fontFamily: 'var(--font-body)' }} className="text-[13px] text-[hsl(var(--color-text-secondary))] mb-5">
                  Describe the hiring scenario. The AI will structure it and ask about any missing information.
                </p>
                <textarea
                  value={scenarioText}
                  onChange={e => setScenarioText(e.target.value)}
                  placeholder="Example: We're looking for a Chief Technology Officer for our Berlin office. The company is scaling rapidly after Series B funding..."
                  rows={8}
                ...
                <button
                  ...
                  style={{ fontFamily: 'var(--font-body)' }}
                >
                  {scenarioParsing || scenarioLoading ? <><Loader2 size={18} className="animate-spin" /> Analyzing...</> : <>Analyze Scenario <ArrowRight size={16} /></>}
                </button>
              </div>
          ...
          {step === 'qa' && (
            <motion.div key="qa" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.35, ease }}>
              <div className="bg-[hsl(var(--color-surface))] border border-[hsl(var(--color-border))] rounded-2xl p-6">
                <h2 style={{ fontFamily: 'var(--font-display)' }} className="text-[22px] text-[hsl(var(--color-text-primary))] mb-2">Fill in Missing Details</h2>
                <p style={{ fontFamily: 'var(--font-body)' }} className="text-[13px] text-[hsl(var(--color-text-secondary))] mb-6">
                  Answer each question to complete the scenario profile.
                ...
                {currentQA ? (
                  <motion.div key={currentQA.field} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, ease }}>
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <p style={{ fontFamily: 'var(--font-body)' }} className="text-[15px] text-[hsl(var(--color-text-primary))] font-medium leading-relaxed">{currentQA.question}</p>
                    </div>
                    <input
                      value={qaAnswer}
                  ...
                  <div className="text-center py-8">
                    ...
                    <button
                      ...
                      style={{ fontFamily: 'var(--font-body)' }}
                    >
                      Proceed to Scoring <BarChart3 size={16} />
                    </button>
                  </div>
                )}
              </div>
            ...
            <motion.div key="scoring" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.35, ease }}>
              {scores.length === 0 && !scoringReport ? (
                <div className="bg-[hsl(var(--color-surface))] border border-[hsl(var(--color-border))] rounded-2xl p-6 text-center">
                  ...
                  <h2 style={{ fontFamily: 'var(--font-display)' }} className="text-[22px] text-[hsl(var(--color-text-primary))] mb-2">Ready to Score</h2>
                  ...
                  <button
                    ...
                    style={{ fontFamily: 'var(--font-body)' }}
                  >
                    {scoring || scorerLoading ? <><Loader2 size={18} className="animate-spin" /> Scoring...</> : <>Run Analysis <ArrowRight size={16} /></>}
                  </button>
                </div>
              ) : (
                <>
                  {scores.length > 0 && (
                    <>
                      <h2 style={{ fontFamily: 'var(--font-display)' }} className="text-[22px] text-[hsl(var(--color-text-primary))] mb-1">Candidate Rankings</h2>
